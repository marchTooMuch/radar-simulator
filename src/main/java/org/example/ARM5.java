...
public class ARM5 extends AntiRadiationMissile {
    int i = 0;
    ImageView view3 ;
    ARM5(double x, double y, ImageView view1, ImageView view2, ImageView view3, int height, double azimuth) {
        super(x,y,view1,view2, view3, height, azimuth,"ARM5");
        this.view3 = view1;
        speed = 4;
    ...
    boolean update(double dt, double targetX, double targetY, Boolean radarOn, Arc sector, Pane interceptorLayer, Pane missileLayer) {
        ...
        double nx = dx / length2;
        double ny = dy / length2;
        dx /= distance.get();
        dy /= distance.get();
        ...
        if (distanceInKm.getValue()>20) {
            height.setValue(15000);
        }
        if(distanceInKm.getValue()<= 75 && distanceInKm.getValue() !=0) {
            firstLine(nx,ny);
        }
        if(distanceInKm.getValue() < 20 && distanceInKm.getValue() != 0) {
            secondLine();
        }
        i++;
        if(i>100000000) {
...
//                }
        ...
        if(Buttons.safeModeOn.get() == false && distance.get() <= 300 && isEngaged == false && FirstTable.rocketsCount > 0) {
            ...
            if (distance.get() < 10) {
                distanceInKm.set(0);height.setValue(0);return true;};
        x += dx * speed * dt;
        y += dy * speed * dt;
    ...
    public static AntiRadiationMissile spawnMissileARM1(Pane missileLayer, Arc sector) {
        ...
        ARM5 m = new ARM5(x, y, view, view2, view3,8000, angle);
//        m.angle = angle;
    ...
    public void secondLine() {
        double a = (3.0/4) * distanceInKm.getValue();
        a*=1000;
        height.setValue(a);
        double slope = 3/4;
